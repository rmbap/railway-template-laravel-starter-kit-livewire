 ...
 * Topbar simples (MVP)
 */
if (!function_exists('topbar_html')) {
 ...
 */
if (!function_exists('ensure_company_id_column')) {
...
if (!function_exists('ensure_companies_table')) {
...
if (!function_exists('ensure_daily_metrics_table')) {
...
if (!function_exists('get_channel_summary')) {
...
Route::middleware(['auth', 'verified'])->group(function () {
            ...
            if ($best && $worst && $best['channel'] !== $worst['channel'] && $best['cpa'] > 0 && $worst['cpa'] !== null) {
                ...
                $html .= '
                    <div style="padding:14px;border:1px solid #ddd;border-radius:12px;">
                        ...
                        <div style="margin-top:6px;color:#555;">(MVP) Diferença de CPA: ~' . number_format(($ratio - 1) * 100, 0) . '%</div>
    ...
    // (mantive /analysis e /simulate fora aqui pra você não ficar com mais um arquivo gigante)
    // ✅ Se você quiser, eu te mando o arquivo COMPLETO com /analysis e /simulate também,
    // mas antes: esse patch já deve fazer o deploy voltar a ficar saudável por causa do /up + guards.
