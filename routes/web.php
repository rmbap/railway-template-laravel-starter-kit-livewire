...
Route::middleware(['auth', 'verified'])->group(function () {
        ...
        <div style="position:sticky;top:0;z-index:50;background:#fff;border-bottom:1px solid #eee;">
          <div style="max-width:1100px;margin:0 auto;padding:12px 14px;display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap;font-family:-apple-system,BlinkMacSystemFont,Segoe UI,Roboto,Arial;">
            <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;">
              ...
              <a href="/planner" style="padding:8px 12px;border-radius:10px;border:1px solid #e5e5e5;background:#fff;color:#111;text-decoration:none;">Planner</a>
            </div>
            <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;">
    ...
    Route::get('/planner', function () {
        ...
        <div style="position:sticky;top:0;z-index:50;background:#fff;border-bottom:1px solid #eee;">
          <div style="max-width:1100px;margin:0 auto;padding:12px 14px;display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap;font-family:-apple-system,BlinkMacSystemFont,Segoe UI,Roboto,Arial;">
            <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;">
              <div style="font-weight:900;">Budget Engine</div>
              <a href="/dashboard" style="padding:8px 12px;border-radius:10px;border:1px solid #e5e5e5;background:#fff;color:#111;text-decoration:none;">Dashboard</a>
              <a href="/company/create" style="padding:8px 12px;border-radius:10px;border:1px solid #e5e5e5;background:#fff;color:#111;text-decoration:none;">Empresa</a>
              <a href="/metrics/create" style="padding:8px 12px;border-radius:10px;border:1px solid #e5e5e5;background:#fff;color:#111;text-decoration:none;">Métricas</a>
              <a href="/analysis" style="padding:8px 12px;border-radius:10px;border:1px solid #e5e5e5;background:#fff;color:#111;text-decoration:none;">Análise</a>
              <a href="/planner" style="padding:8px 12px;border-radius:10px;background:#111;color:#fff;text-decoration:none;">Planner</a>
            </div>
            <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;">
              <a href="/settings" style="padding:8px 12px;border-radius:10px;border:1px solid #e5e5e5;background:#fff;color:#111;text-decoration:none;">Settings</a>
              <form method="POST" action="/logout" style="display:inline;margin:0;">
     ...
     * PLANNER - RESULTADO
     */
    Route::post('/planner', function (Request $request) {
        ...
        if ($rows->count() === 0) {
            ...
            <div style="font-family:-apple-system,BlinkMacSystemFont,Segoe UI,Roboto,Arial; max-width:720px; margin:24px auto; padding:0 14px;">
                <h1>Planner de orçamento</h1>
                <p>Nenhuma métrica encontrada para calcular o plano.</p>
        ...
        foreach ($rows as $r) {
            ...
            $roas = $rev / $spend;
            $leadsPerReal = $conv / $spend;

            $score = $goal === 'revenue' ? $roas : $leadsPerReal;
            ...
            $channels[] = [
                ...
                'historical_leads' => $conv,
                'historical_revenue' => $rev,
                'roas' => $roas,
        ...
        if (count($channels) === 0 || $totalScore <= 0) {
            return $nav . '
            <div style="font-family:-apple-system,BlinkMacSystemFont,Segoe UI,Roboto,Arial; max-width:720px; margin:24px auto; padding:0 14px;">
                <h1>Planner de orçamento</h1>
        ...
        usort($channels, fn($a, $b) => $b['allocated_budget'] <=> $a['allocated_budget']);

        $goalLabel = $goal === 'revenue' ? 'Maximizar receita' : 'Maximizar leads';

        $html = $nav . '
        <div style="font-family:-apple-system,BlinkMacSystemFont,Segoe UI,Roboto,Arial; max-width:980px; margin:24px auto; padding:0 14px;">
            <h1>Planner de orçamento</h1>
            <p><b>Objetivo:</b> '.$goalLabel.'</p>
        ...
        foreach ($channels as $channel) {
            $html .= '
                <tr style="border-bottom:1px solid #f2f2f2;">
                    <td style="padding:10px 8px;">'.htmlspecialchars($channel['channel']).'</td>
